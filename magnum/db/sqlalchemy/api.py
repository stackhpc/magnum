...
class Connection(api.Connection):
    ...
    def _add_clusters_filters(self, query, filters):
        ...
        filter_dict = {
            filter_name: filters[filter_name] for filter_name in filter_names
        }
    ...
    def get_cluster_template_list(
        ...
        return _paginate_query(
            models.ClusterTemplate, limit, marker, sort_key, sort_dir, query
        )
    ...
    def _is_publishing_cluster_template(self, values):
        if len(values) == 1 and (
            ...
            or ("hidden" in values)
            or ("tags" in values and values["tags"] is not None)
        ):
            return True
        return False
    ...
    def _do_update_cluster_template(self, cluster_template_id, values):
        with _session_for_write() as session:
            ...
            if self._is_cluster_template_referenced(session, ref["uuid"]):
                ...
                if not self._is_publishing_cluster_template(values) and list(
                    values.keys()
                ) != ["name"]:
                    raise exception.ClusterTemplateReferenced(
                        clustertemplate=cluster_template_id
