...
class Driver(driver.Driver):
    ...
    def provides(self):
        return [
            {'server_type': 'vm',
             # NOTE(johngarbutt) means any cluster api supported image
             'os': 'cluster_api',
             'coe': 'kubernetes'},
        ]
    ...
    def resize_cluster(self, context, cluster, resize_manager, node_count, nodes_to_remove, nodegroup=None):
        raise Exception("don't support removing nodes this way yet")

    def upgrade_cluster(self, context, cluster, cluster_template, max_batch_size, nodegroup, scale_manager=None,
                        rollback=False):
